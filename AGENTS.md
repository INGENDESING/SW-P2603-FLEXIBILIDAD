...
**Excepción**: tareas con pipeline maestro ya aprobado y documentado (p. ej. 
`generadorinf.md` para informes LaTeX) no requieren plan nuevo en `task/todo.md`; 
basta confirmar la ejecución con el usuario y registrar el resultado al finalizar.
</fase_1_analisis>
...
<cierre_de_sesion>
Este proyecto tiene memoria persistente: bóveda Obsidian en `boveda/` y skills 
`inicializar`/`memoria` en `.kimi-code/skills/`. Al iniciar sesión, seguir 
`inicializar.md`; al cerrar, actualizar la bóveda con la skill `memoria`.
